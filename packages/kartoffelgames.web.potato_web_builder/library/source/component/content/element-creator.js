...
class ElementCreator {
     ...
     * @returns
     */
    static createElement(pXmlElement, pParentElement) {
        const lTagname = typeof pXmlElement === 'string' ? pXmlElement : pXmlElement.tagName;
        let lNamespace = typeof pXmlElement === 'string' ? null : pXmlElement.namespace;
        // On custom element
        ...
        const lElementRestriction = ElementCreator.mElementRestriction.get(lTagname.toLowerCase());
    ...
    static createText(pText) {
        ...
        const lDecodedText = pText.replace(/&#(\d+);/g, (_pFullMatch, pCharAsDecimal) => {
...
ElementCreator.mElementRestriction = (() => {
    const lElementRestriction = new core_data_1.Dictionary();
