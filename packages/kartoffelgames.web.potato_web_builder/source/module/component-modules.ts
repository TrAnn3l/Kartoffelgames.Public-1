...
import { ModuleType } from '../enum/module-type'; import { Injection, InjectionConstructor } from '@kartoffelgames/core.dependency-injection';
...
export class ComponentModules {
    ...
    public static addModule(pAttributeModule: PwbStaticAttributeModuleConstructor | PwbManipulatorAttributeModuleConstructor | PwbExpressionModuleConstructor): void {
        if (pAttributeModule.moduleType === ModuleType.Static) {
            ComponentModules.mStaticModuleList.push(pAttributeModule);
        } else if (pAttributeModule.moduleType === ModuleType.Manipulator) {
            ComponentModules.mManipulatorModuleList.push(pAttributeModule);
        } else if (pAttributeModule.moduleType === ModuleType.Expression) {
            ComponentModules.mExpressionModuleList.push(pAttributeModule);
        }
    }
     ...
     */
    public static checkTemplateIsManipulator(pTemplate: BaseXmlNode): boolean {
        // Get manipulator module from template and check if it exists.
        return typeof ComponentModules.getManipulatorModuleConstructor(pTemplate) !== 'undefined';
    }
    ...
    public static getStaticModule(pTargetElement: HtmlContent, pTargetTemplate: XmlElement, pValues: LayerValues, pAttribute: XmlAttribute, pComponentHandler: ComponentManager, pAccessFilter?: AttributeModuleAccessType): IPwbStaticAttributeModule | undefined {
        ...
        // Find static module.
        const lModuleConstructor: PwbStaticAttributeModuleConstructor = ComponentModules.mStaticModuleList.find(lFindFn);
     ...
     * @param pTemplate - Template element.
     */
    private static getManipulatorModuleConstructor(pTemplate: BaseXmlNode): PwbManipulatorAttributeModuleConstructor | undefined {
        if(!(pTemplate instanceof XmlElement)){
            return undefined;
        }
        ...
        return ComponentModules.mManipulatorModuleList.find(lFindFn);
    }
     ...
     * @param pTemplate - Template element.
     */
    public templateUsesMultiplicatorModule(pTemplate: BaseXmlNode): boolean {
        return ComponentModules.checkTemplateIsManipulator(pTemplate);
    }
    ...
    public getExpressionModule(pTargetNode: HtmlContent | Text, pAttributeName: string | null, pValue: string, pValueHandler: LayerValues, pComponentHandler: ComponentManager): IPwbExpressionModule {
        return ComponentModules.getExpressionModule(this.mExpressionModule, pTargetNode, pAttributeName, pValue, pValueHandler, pComponentHandler);
    }
    ...
    public getManipulatorModule(pTargetTemplate: XmlElement, pValues: LayerValues, pComponentHandler: ComponentManager): IPwbManipulatorAttributeModule | undefined {
        return ComponentModules.getManipulatorModule(pTargetTemplate, pValues, pComponentHandler);
    }
    ...
    public getStaticModule(pTargetElement: HtmlContent, pTargetTemplate: XmlElement, pValues: LayerValues, pAttribute: XmlAttribute, pComponentHandler: ComponentManager, pAccessFilter?: AttributeModuleAccessType): IPwbStaticAttributeModule | undefined {
        return ComponentModules.getStaticModule(pTargetElement, pTargetTemplate, pValues, pAttribute, pComponentHandler, pAccessFilter);
    }
}
